...
public class UiWatchers {
     ...
     * executed when the framework is waiting for a control to appear. Waiting
     * may be the cause of an unexpected dialog on the screen and it is the time
     * when the framework runs the registered watchers. This is a sample watcher
     * looking for ANR and crashes. it closes it and moves on. You should create
     * your own watchers and handle error logging properly for your type of
     * tests.
     */
    ...
    public void registerAcceptSSLCertWatcher() {
        UiDevice.getInstance().registerWatcher("SSLCERTERROR", new UiWatcher() {
            @Override
            public boolean checkForCondition() {
                UiObject continueButton = new UiObject(new UiSelector().className("android.widget.Button").packageName("com.android.browser").text("Continue"));
