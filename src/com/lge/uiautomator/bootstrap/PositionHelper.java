...
public abstract class PositionHelper {
     ...
     * (by passing in a double between 0 and 1) or absolute position based on
    ...
    /**
     * Translates coordinates relative to an element rectangle into absolute
     * coordinates.
     *
     * @param point A point in relative coordinates.
     * @param displayRect The display rectangle to which the point is relative.
     * @param offsets X and Y values by which to offset the point. These are
     *            typically the absolute coordinates of the display rectangle.
     * @param shouldCheckBounds Throw if the translated point is outside
     *            displayRect?
     * @return
     * @throws UiObjectNotFoundException
     * @throws InvalidCoordinatesException
     */
    public static Point getAbsolutePosition(final Point point, final Rect displayRect, final Point offsets, final boolean shouldCheckBounds) throws InvalidCoordinatesException {
        ...
        if (shouldCheckBounds && !displayRect.contains(absolutePosition.x.intValue(), absolutePosition.y.intValue())) {
            throw new InvalidCoordinatesException("Coordinate " + absolutePosition.toString() + " is outside of element rect: " + displayRect.toShortString());
        }

        return absolutePosition;
    }

    public static Point getDeviceAbsPos(final Point point) throws UiObjectNotFoundException, InvalidCoordinatesException {
