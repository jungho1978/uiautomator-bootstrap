...
import com.lge.uiautomator.bootstrap.utils.ClassInstancePair;
import com.lge.uiautomator.bootstrap.utils.ElementHelpers;
import com.lge.uiautomator.bootstrap.utils.ReflectionUtils;
import com.lge.uiautomator.bootstrap.utils.UiAutomatorParser;
...
import com.lge.uiautomator.exceptions.UiSelectorSyntaxException;
...
public class Find extends CommandHandler {
     ...
     */
    private JSONArray elementsToJSONArray(final List<AndroidElement> els) throws JSONException {
     ...
     * @throws JSONException
     */
    private AndroidCommandResult execute(final AndroidCommand command, final boolean isRetry) throws JSONException {
        ...
        Logger.debug("Finding " + text + " using " + strategy.toString() + " with the contextId: " + contextId + " multiple: " + multiple);
        boolean found = false;
        try {
            Object result = null;
            ...
            if (!multiple) {
                ...
                for (final UiSelector sel : selectors) {
                    // With multiple selectors, we expect that some elements may
                    // not
                    // exist.
                    try {
                        Logger.debug("Using: " + sel.toString());
                        final List<AndroidElement> elementsFromSelector = fetchElements(sel, contextId);
            ...
            if (!found) {
                if (!isRetry) {
                    ...
                    // some control updates fail to trigger AccessibilityEvents,
                    // resulting
                    // in stale AccessibilityNodeInfo instances. In these cases,
                    // UIAutomator
                    // will fail to locate visible elements. As a work-around,
                    // force clear
                    // the AccessibilityInteractionClient's cache and search
                    // again. This
                    // technique also appears to make Appium's searches conclude
                    // more quickly.
                    // See Appium issue #4200
                ...
                if (!multiple) {
                    // If there are no results and we've already retried, return
                    // an error.
    ...
    /**
     * Get the element from the {@link AndroidElementsHash} and return the
     * element id using JSON.
     *
     * @param sel A UiSelector that targets the element to fetch.
     * @param contextId The Id of the element used for the context.
     * @return JSONObject
     * @throws JSONException
     * @throws ElementNotFoundException
     */
    private JSONObject fetchElement(final UiSelector sel, final String contextId) throws JSONException, ElementNotFoundException {
    ...
    /**
     * Get an array of AndroidElement objects from the
     * {@link AndroidElementsHash}
     *
     * @param sel A UiSelector that targets the element to fetch.
     * @param contextId The Id of the element used for the context.
     ...
     */
    private ArrayList<AndroidElement> fetchElements(final UiSelector sel, final String contextId) throws UiObjectNotFoundException {
     ...
     * @param many Boolean that is either only one element (false), or many
     *            (true)
     * @return UiSelector
     * @throws InvalidStrategyException
     * @throws ElementNotFoundException
     */
    private List<UiSelector> getSelectors(final Strategy strategy, final String text, final boolean many, final String contextId) throws InvalidStrategyException, ElementNotFoundException, UiSelectorSyntaxException, ParserConfigurationException, InvalidSelectorException {
        ...
        case ID:
            ...
            if (API_18) {
                if (resourceIdRegex.matcher(text).matches()) {
                    ...
                    // either it's prefixed with the app package or the android
                    // system page.
        ...
        case CSS_SELECTOR:
        default:
            throw new InvalidStrategyException("Sorry, we don't support the '" + strategy.getStrategyName() + "' locator strategy yet");
        }
    ...
    private List<UiSelector> getXPathSelectors(final String expression, final boolean multiple, String contextId) throws ElementNotFoundException, ParserConfigurationException, InvalidSelectorException {
