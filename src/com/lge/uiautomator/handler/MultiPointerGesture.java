...
public class MultiPointerGesture extends CommandHandler {
    ...
    private double computeLongestTime(final JSONArray actions) throws JSONException {
        ...
        for (int i = 0; i < actions.length(); i++) {
            ...
            final double endTime = gestures.getJSONObject(gestures.length() - 1).getDouble("time");
            if (endTime > max) {
                max = endTime;
            }
        }
    ...
    private PointerCoords createPointerCoords(final JSONObject obj) throws JSONException {
        ...
        try {
            ...
            if (command.isElementCommand()) {
                ...
                if (API_18) {
                    ...
                    return new AndroidCommandResult(WDStatus.UNKNOWN_ERROR, "Cannot perform multi pointer gesture on device below API level 18");
                }
            }
    ...
    private PointerCoords[] gesturesToPointerCoords(final double maxTime, final JSONArray gestures) throws JSONException {
        // gestures, e.g.:
        // [
    ...
    private PointerCoords[][] parsePointerCoords(final AndroidCommand command) throws JSONException {
        final JSONArray actions = (org.json.JSONArray)command.params().get("actions");
