...
public class Pinch extends CommandHandler {
    ...
    public AndroidCommandResult execute(final AndroidCommand command) throws JSONException {
        ...
        try {
            ...
            if (el == null) {
                return getErrorResult("Could not find an element with elementId: " + params.get("elementId"));
            }
        } catch (final Exception e) { // JSONException, NullPointerException,
                                      // etc.
            return getErrorResult("Unknown error:" + e.getMessage());
        }

        Logger.debug("Pinching " + direction + " " + percent.toString() + "%" + " with steps: " + steps.toString());
        boolean res;
