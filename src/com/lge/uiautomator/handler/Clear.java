 ...
 *
 * UiAutomator method clearText is flaky hence overriding it with custom
 * implementation.
 */
public class Clear extends CommandHandler {

    /*
     * Trying to select entire text with correctLongClick and increasing time
     * intervals. Checking if element still has text in them and and if true
     * falling back on UiAutomator clearText
     * 
    ...
    public AndroidCommandResult execute(final AndroidCommand command) throws JSONException {
        if (command.isElementCommand()) {
            try {
                ...
                if (hasHintText(el)) {
                    Logger.debug("Text remains after clearing, " + "but it appears to be hint text.");
                    return getSuccessResult(true);
                }

                // next try to select everything and delete
                Logger.debug("Clearing text not successful. Attempting to clear " + "by selecting all and deleting.");
                ...
                Logger.debug("Clearing text not successful. Attempting to clear " + "by sending delete keys.");
                ...
                if (!el.getText().isEmpty()) {
                    ...
                    if (hasHintText(el)) {
                        ...
                        Logger.debug("Exhausted all means to clear text but '" + el.getText() + "' remains.");
    ...
    private boolean selectAndDelete(AndroidElement el) throws UiObjectNotFoundException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
    ...
    private boolean sendDeleteKeys(AndroidElement el) throws UiObjectNotFoundException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        ...
        while (!el.getText().isEmpty() && !tempTextHolder.equalsIgnoreCase(el.getText())) {
            ...
            for (int key : new int[] { KeyEvent.KEYCODE_DEL, KeyEvent.KEYCODE_FORWARD_DEL }) {
                ...
                KeyEvent deleteEvent = new KeyEvent(eventTime, eventTime, KeyEvent.ACTION_DOWN, key, 0, 0, KeyCharacterMap.VIRTUAL_KEYBOARD, 0, 0, InputDevice.SOURCE_KEYBOARD);
    ...
    private boolean hasHintText(AndroidElement el) throws UiObjectNotFoundException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        // to test if the remaining text is hint text, try sending a single
        // delete key and testing if there is any change.
        // ignore the off-chance that the delete silently fails and we get a
        // false
        // positive.
